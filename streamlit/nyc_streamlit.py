...
st.header("What is the distribution of property proce?")
st.write("""Select a custom price range from the side bar to update the histogram below displayed as s Plotly chart using
...
st.header("What is the distribution of availability in various neighbourhoods?")
st.write("Using a radio button restricts selection to only one option at a time")
st.write("Notice how we use a static table below instead of a dataframe. \
    Unlike a dataframe, if content overflows out of the section margin, \
...
@st.cache
def get_availability(show_exp, neighbourhood):
    return df.query(f"""neighbourhood_group==@neighbourhood{show_exp}\
        and availability_365>0""").availability_365.describe(\
            percentiles=[.1, .25, .5, .75, .9, .99]).to_frame().T

st.table(get_availability(show_exp, neighbourhood))
...
st.markdown("_**Note:** There are 18431 records with `availability_365` 0 (zero), which we've ignores._")

df.query("availability_365>0").groupby("neighbourhood_group")\
    .availability_365.mean().plot.bar(rot=0).set(title="Average availability by neighbourhood group",
    xlabel="Neighbourhood group", ylabel="Avg. Availability (in no. of days)")
st.pyplot()

st.header("Properties by number of reviews")
st.write("Enter a range of numbers in the sidebar to view the properties whose review count falls in that range.")
...
st.write("486 is the highgest number of reviews and two properties have it. Both are in the East Elmhurst \
    neighbourhood and are private rooms with prices $65 and $45. \
...
st.write("Click below to celebrate getting through the tutorial")
